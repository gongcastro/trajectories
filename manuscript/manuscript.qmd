...
Data and materials for this research are available on the Open Science Framework (https://osf.io/hy984/?view_only=50038c88af2a4b7ab37563078f8952f4 [TEMPORARY LINK TO ANONIMISED OSF FOLDER]), and code is available on GitHub (https://anonymous.4open.science/r/cognate-beginnings-anonymous/README.md [TEMPORARY LINK TO ANONIMISED OSF REPOSITORY]). For reproducibility, a Docker image of the RStudio session is available on DockerHub ([LINK REDACTED FOR BLIND REVIEW]).
...
participants_sex <- c(table(distinct(participants, id, sex)$sex),
  missing = sum(is.na(distinct(participants, id, sex)$sex))
)
```
...
Participants were residents in [CITY NAME REDACTED FOR BLIND REVIEW] (Catalonia, Spain). Data collection took place between March 30th, 2020 and October 31th, 2022. Participants were part of the database of the [LABORATORY NAME REDACTED FOR BLIND REVIEW] and were contacted by e-mail or phone if their child was aged between `r floor(min(responses$age))` and `r ceiling(max(responses$age))` months. Upon consent, families were sent a link to the questionnaire via e-mail, which they filled from a computer, laptop, or mobile device. Filling the questionnaire took 30 minutes approximately. After completion, families were rewarded with a token of appreciation. Before filling out the vocabulary questionnaire, parents completed a brief demographic survey and a language exposure questionnaire. The demographic survey collection information about the childs' sex and caregivers' educational attainments.
